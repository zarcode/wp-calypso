...
describe( 'actions', () => {
	...
	describe( 'Actions creators', () => {
		it( '#createDomainsReceiveAction()', () => {
			const { domains } = wpcomResponse;
			const action = createDomainsReceiveAction( siteId, domains );
			expect( action ).to.eql( ACTION_SITE_DOMAIN_RECEIVE );
		} );

		it( '#createDomainsRequestAction()', () => {
			const action = createDomainsRequestAction( siteId );
			expect( action ).to.eql( ACTION_SITE_DOMAIN_REQUEST );
		} );

		it( '#createDomainsRequestSuccessAction()', () => {
			const action = createDomainsRequestSuccessAction( siteId );
			expect( action ).to.eql( ACTION_SITE_DOMAIN_REQUEST_SUCCESS );
		} );

		it( '#createDomainsRequestFailureAction()', () => {
			const action = createDomainsRequestFailureAction( siteId, errorResponse );
	...
	describe( '#fetchSiteDomains() - failure', () => {
		...
		it( 'should dispatch REQUEST_FAILURE action when request failed', () => {
			...
			const requestAction = createDomainsRequestAction( siteId );
			const failureAction = createDomainsRequestFailureAction( siteId, message );
